<template>
  <div class="intro-y mt-8 flex items-center">
    <h2 v-if="!route.query.code" class="mr-auto text-lg font-medium">
      {{ $t("leaveApplication.resign") }}
    </h2>
    <h2 v-else class="mr-auto text-lg font-medium">
      {{ $t("leaveApplication.view") }}
    </h2>
  </div>
  <!-- BEGIN: Page Layout 1 -->
      ...
      <div class="intro-y box p-5">
        <div>
          <label class="form-label font-medium text-base">
            {{ $t("leaveApplication.IDrequire") }}</label
          >
          <input
        ...
        <div class="mt-3 flex flex-col items-start">
          <label class="form-label font-medium text-base"
            >{{ $t("leaveApplication.requestDate") }}
  ...
  <div class="mt-5 text-danger">
    <div class="flex flex-row">
      <div class="mr-3">※</div>
      <div>
        {{ $t("leaveApplication.note1") }}
      </div>
    </div>
    <div class="flex flex-row">
      <div class="mr-3">※</div>
      <div>
        {{ $t("leaveApplication.note2") }}
      </div>
    </div>
    <div class="flex flex-row">
      <div class="mr-3">※</div>
      <div>
        {{ $t("leaveApplication.note3") }}
      </div>
    </div>
    <div class="flex flex-row">
      <div class="mr-3">※</div>
      <div>
        {{ $t("leaveApplication.note4") }}
      </div>
    </div>
  </div>
  <!-- END: Page Layout 1 -->
  ...
  <div class="mt-5">
    <Confirm
      ...
      :type-validate="'confirm-leave-application'"
      :is-view="!(!route.query.code || isUpdateDraft || isSave)"
    ></Confirm>
  </div>
  <!-- END: Page Layout 4 -->
  ...
  <div class="grid grid-cols-12 mt-5 gap-6">
    <div class="intro-y col-span-12">
      <div class="text-right">
        <router-link :to="{ path: '/request/leave-application/list' }">
          <button
...
let getFlowDepartment = () => {
  let successCallback = (response) => {
    ...
    flowDepartmentStart.value = dataResponse.leave_request;
  };
  let errorCallback = () => {};
  let payload = {
...
let getDetail = () => {
  ...
  leaveApplicationStore.get(payload);
};
...
let registerValidate = () => {
  leaveApplicationStore.registerData = true;
  leaveApplicationStore.checkLogRequest.validateFlow = true;
  leaveApplicationStore.checkConfirm.validateFlow = true;
};

let saveValidate = () => {
  leaveApplicationStore.saveData = true;
  leaveApplicationStore.checkLogRequest.validateFlow = true;
  leaveApplicationStore.checkConfirm.validateFlow = true;
};
...
let resign = async () => {
  ...
  leaveApplicationStore.create(payload);
};
...
let save = () => {
  ...
  leaveApplicationStore.update(payload);
};
...
watch(
  () => [
    leaveApplicationStore.registerData,
    leaveApplicationStore.saveData,
  ...
  () => {
    ...
    if (
      !leaveApplicationStore.checkLogRequest.check ||
      !leaveApplicationStore.checkConfirm.check ||
      !check
    ) {
      return;
    }
    if (leaveApplicationStore.registerData) {
      resign();
      leaveApplicationStore.registerData = false;
      leaveApplicationStore.checkLogRequest.check = false;
      leaveApplicationStore.checkConfirm.check = false;
    }
    if (leaveApplicationStore.saveData) {
      save();
      leaveApplicationStore.saveData = false;
      leaveApplicationStore.checkLogRequest.check = false;
      leaveApplicationStore.checkConfirm.check = false;
    }
  },
