<template>
    ...
    >
      <div class="flex w-full md:w-1/3">
        <div class="w-full relative text-slate-500">
          <input
            v-model="q"
            :placeholder="$t('leaveApplication.valueSearch')"
      ...
      <div class="w-auto mt-3 md:mt-0 flex flex-row">
        <router-link :to="{ path: '/request/leave-application/add' }">
      ...
      >
        <template #row="{ row, index, leng }">
          <tr class="intro-x" @dblclick="viewItem(row)">
            ...
            <td class="text-left" :rowspan="leng">
              <div class="block-textName">
                {{ helper.getTypeRequest(row.log?.[0]?.type) ?? "" }}
              </div>
            </td>
            <td class="text-left">
              <div class="block-textName">
            ...
            <td class="text-left" :rowspan="leng">
              <div class="whitespace-nowrap">
                <textarea
                  class="remarks-text input-boxshadow"
                  :value="row.remarks"
  ...
  <ExportFile :type-export="'leave_request'" v-model:show-export="showExport">
  </ExportFile>
</template>
<script>
export default {
  name: "list-leave-application",
};
</script>
<script setup>
import { TYPE_ADMIN, STATUS_CONFIRM } from "@/config/constants.js";
...
const resizeTable = ref(
  JSON.parse(localStorage.getItem("listLeaveApplication"))
);
const config = ref({
  action: "leave_application",
  fitResize: "listLeaveApplication",
  q: {
    keySearch: "",
    typeSearch: [
      "code",
  ...
  headers: [
    ...
    {
      label: i18n.global.t("leaveApplication.typeRequest"),
    },
    {
...
let viewItem = (row) => {
  router.push({
    ...
    query: {
      code: row.code,
      type: "view",
      pageName: i18n.global.t("timeReport.view"),
    },
  });
};
...
let deleteItem = (row) => {
  let confirmCallback = () => {
    ...
    leaveApplicationStore.delete(payload);
  };
  let payloadConfirm = {
