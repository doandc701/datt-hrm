<template>
    ...
    >
      <div class="flex w-full md:w-1/3">
        <div class="w-full relative text-slate-500">
          <input
            v-model="q"
            :placeholder="$t('timeReport.valueSearch')"
        ...
        <div class="flex flex-row" v-if="visibleAction">
          ...
          <div class="form-check mr-2 mt-2 sm:mt-0">
            ...
            /><label class="form-check-label" for="radio-switch-2"
              >{{ $t("timeReport.filter2") }}
            </label>
          </div>
        </div>
      </div>
      <div class="w-auto mt-3 md:mt-0 flex flex-row">
        <router-link :to="{ path: '/request/time-report/add' }">
  ...
  <ExportFile
    :type-export="'trip_working_time_report'"
...
const resizeTable = ref(JSON.parse(localStorage.getItem("listTimeReport")));
const config = ref({
  action: "list_time_report",
  fitResize: "listTimeReport",
  q: {
    keySearch: "",
    typeSearch: [
      "code",
...
let deleteItem = (row) => {
  let confirmCallback = () => {
    ...
    timeReportStore.delete(payload);
  };
  let payloadConfirm = {
