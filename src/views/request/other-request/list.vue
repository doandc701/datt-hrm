<template>
    ...
    >
      <div class="flex w-full md:w-1/3">
        <div class="w-full relative text-slate-500">
          <input
            v-model="q"
            :placeholder="$t('otherRequest.valueSearch')"
      ...
      <div class="w-auto mt-3 md:mt-0 flex flex-row">
        <router-link :to="{ path: '/request/other-request/add' }">
  ...
  <ExportFile :type-export="'other_request'" v-model:show-export="showExport">
  </ExportFile>
</template>
<script>
export default {
  name: "list-overtime-early",
};
</script>
<script setup>
...
const resizeTable = ref(JSON.parse(localStorage.getItem("listOtherRequest")));
const config = ref({
  action: "list_other_request",
  fitResize: "listOtherRequest",
  q: {
    keySearch: "",
    typeSearch: ["code", "request_name", "request_date", "office_code"],
  },
  dataFilterKey: {
    draft: "",
  },
  ...
  headers: [
    ...
    {
      label: i18n.global.t("confirm.userConfirm"),
    },
    {
      label: i18n.global.t("otherRequest.statusConfirm"),
    },
    {
      label: i18n.global.t("overTime.remarks"),
    },
  ],
  sort: {
    field: "code",
...
let editItem = (row) => {
  router.push({
    ...
    query: {
      code: row.code,
      type: "edit",
      pageName: i18n.global.t("otherRequest.detail"),
    },
  });
};
...
let deleteItem = (row) => {
  let confirmCallback = () => {
    ...
    otherRequestStore.delete(payload);
  };
  let payloadConfirm = {
