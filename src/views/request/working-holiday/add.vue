<template>
  <div class="intro-y mt-8 flex items-center">
    <h2 v-if="!route.query.code" class="mr-auto text-lg font-medium">
      {{ $t("workingHoliday.registerRequest") }}
    </h2>
    <h2 v-else class="mr-auto text-lg font-medium">
      {{ $t("workingHoliday.detailRequest") }}
    </h2>
  </div>
  <!-- BEGIN: Page Layout 1 -->
  ...
  <div class="flex flex-row mt-5 text-danger">
    <div>
      <div>{{ $t("workingHoliday.note1") }}</div>
      <div>{{ $t("workingHoliday.note2") }}</div>
      <div>{{ $t("workingHoliday.note3") }}</div>
    </div>
  </div>
  <!-- END: Page Layout 1 -->
  <!-- BEGIN: Page Layout 2 -->
  ...
  <div class="grid grid-cols-12 mt-5 gap-6">
    <div class="intro-y col-span-12">
      <div class="text-right">
        <router-link :to="{ path: '/request/working-holiday/list' }">
          <button
...
import LogRequest from "./request/log-request.vue";
...
//request
const requestBy = ref(JSON.parse(localStorage.getItem("userInfo")).name);
...
let getFlowDepartment = () => {
  let successCallback = (response) => {
    ...
    flowDepartmentStart.value = dataResponse.working_holiday_request;
  };
  let errorCallback = () => {};
  let payload = {
...
let getDetail = () => {
  ...
  workingHolidayStore.get(payload);
};
...
let registerValidate = () => {
  workingHolidayStore.registerData = true;
  workingHolidayStore.checkLogRequest.validateFlow = true;
  workingHolidayStore.checkConfirm.validateFlow = true;
};

let saveValidate = () => {
  workingHolidayStore.saveData = true;
  workingHolidayStore.checkLogRequest.validateFlow = true;
  workingHolidayStore.checkConfirm.validateFlow = true;
};
...
let resign = () => {
  ...
  workingHolidayStore.create(payload);
};
...
let save = () => {
  ...
  workingHolidayStore.update(payload);
};
...
watch(
  () => [
    workingHolidayStore.registerData,
    workingHolidayStore.saveData,
  ...
  () => {
    ...
    if (
      !workingHolidayStore.checkLogRequest.check ||
      !workingHolidayStore.checkConfirm.check
    ) {
      return;
    }
    if (workingHolidayStore.registerData) {
      resign();
      workingHolidayStore.registerData = false;
      workingHolidayStore.checkLogRequest.check = false;
      workingHolidayStore.checkConfirm.check = false;
    }

    if (workingHolidayStore.saveData) {
      save();
      workingHolidayStore.saveData = false;
      workingHolidayStore.checkLogRequest.check = false;
      workingHolidayStore.checkConfirm.check = false;
    }
  },
