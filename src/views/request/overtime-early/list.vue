<template>
    ...
    >
      <div class="flex w-full md:w-1/3">
        <div class="w-full relative text-slate-500">
          <input
            v-model="q"
            :placeholder="$t('overtimeEarly.valueSearch')"
      ...
      <div class="w-auto mt-3 md:mt-0 flex flex-row">
        <router-link :to="{ path: '/request/overtime-early/add' }">
  ...
  <ExportFile
    :type-export="'overtime_request_leaving_early'"
...
const resizeTable = ref(JSON.parse(localStorage.getItem("listOverTimeEarly")));
const config = ref({
  action: "list_overtime_early",
  fitResize: "listOverTimeEarly",
  q: {
    keySearch: "",
    typeSearch: ["code", "request_name", "request_date"],
  },
  dataFilterKey: {
    draft: "",
  },
...
let editItem = (row) => {
  router.push({
    ...
    query: {
      code: row.code,
      type: "edit",
      pageName: i18n.global.t("overtimeEarly.detailRequest"),
    },
  });
};
