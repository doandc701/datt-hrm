<template>
  <h2 class="intro-y text-lg font-medium mt-10">{{ $t("overTime.list") }}</h2>
    ...
    >
      <div class="flex w-full md:w-1/3">
        <div class="w-full relative text-slate-500">
          <input
            v-model="q"
            :placeholder="$t('overTime.valueSearch')"
      ...
      <div class="w-auto mt-3 md:mt-0 flex flex-row">
        <router-link :to="{ path: '/request/over-time/add' }">
...
const resizeTable = ref(JSON.parse(localStorage.getItem("listOverTime")));
const config = ref({
  action: "list_overtime",
  fitResize: "listOverTime",
  q: {
    keySearch: "",
...
let editItem = (row) => {
  router.push({
    ...
    query: {
      code: row.code,
      type: "edit",
      pageName: i18n.global.t("overTime.editOverTime"),
    },
  });
};
