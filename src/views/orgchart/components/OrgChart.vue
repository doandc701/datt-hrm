...
<script>
import OrganizationChartNode from "@/views/orgchart/components/OrganizationChartNode.vue";
...
export default {
  ...
  watch: {
    resetView(value) {
      if (value) {
        this.$emit("update:resetView", false);
        this.resetViewChart(value);
      }
    },
  },
  methods: {
    resetViewChart() {
      this.$nextTick(() => {
        this.transformVal = `matrix(${this.scaleValue},0,0,${
          this.scaleValue
        },-${this.$refs.chartOrgchart.clientWidth / 2 - 100},0`;
      });
    },
...
.chartOrgchart {
  ...
  display: inline-block;
  //border: 1px dashed white; padding: 1.25rem;
}
