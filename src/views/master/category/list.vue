    ...
    >
      <div class="flex w-full sm:w-1/3">
        <div class="relative w-full text-slate-500">
          <input
            v-model="q"
            :placeholder="$t('category.valueSearch')"
...
const config = ref({
  ...
  headers: [
    ...
    {
      sort: "name",
      label: i18n.global.t("category.name"),
    },
    {
      label: i18n.global.t("category.color"),
    },
  ],
  sort: {
    field: "id",
...
let editItem = (row) => {
  router.push({
    ...
    query: {
      code: row.id,
      type: "edit",
      pageName: i18n.global.t("category.editCategory"),
    },
  });
};
let deleteItem = (row) => {
  let confirmCallback = () => {
    ...
    categoryCalendarStore.delete(payload);
  };
  let payloadConfirm = {
