export default {
  ...
  mounted(el, { value }) {
    ...
    ths.forEach((th, index) => {
      ...
      bar.style.left = nextTh.offsetLeft - 2 + "px";
      bar.style.top = 0;
    ...
    document.addEventListener("mouseup", () => {
      ...
      let thWiths = [];
      ths.forEach((th) => {
        thWiths.push(th.offsetWidth + "px");
      });
      thWiths.pop();
      localStorage.setItem(value, JSON.stringify(thWiths));
    });
    ...
    const handleResize = (e) => {
      if (moving) {
        ...
        bar.style.left = nextTh.offsetLeft - 2 + e.movementX + "px";
      }
    };
