...
import { getListFacilities } from "@/utils/select/facilities";

export default {
  name: "select-multi-facilities",
...
const eventFacilities = useFacilityStore();
...
const props = defineProps({
  id: {
    type: String,
    required: false,
    default: "popper-select-facilities-multi",
  },
  code: {
...
const loadmore = () => {
  ...
  if (queryString.value) {
    ...
    getListFacilities(getQueryString(), page.value);
  }
};
...
watch(
  () => [apiStore.listFacilities, props.config.defaultOptions],
  () => {
    options.value = _.cloneDeep(apiStore.listFacilities);
    defaultOptions.value = _.cloneDeep(apiStore.listFacilities);
    page.value = _.cloneDeep(apiStore.pageFacilities);
  ...
  () => {
    ...
    if (value.value.length > 0) {
      value.value.forEach((item) => {
        let data = options.value.find((element) => element.id === item);
        if (data) {
